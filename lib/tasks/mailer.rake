...
task :send_one_schedule_mail => :environment do
  ...
  @users=User.joins(:roles).where(roles: {role_name: "Get Schedule Email"}).map(&:user_email).join(",")
  UserMailer.admin_schedule_email(@users,@candidate.schedule).deliver 
end
...
task :send_one_update_schedule_mail => :environment do
  ...
  @users=User.joins(:roles).where(roles: {role_name: "Get Schedule Email"}).map(&:user_email).join(",")
  UserMailer.admin_update_schedule_email(@users,@candidate.schedule).deliver 
end
...
task :send_schedule_mail => :environment do
  ...
  @users=User.joins(:roles).where(roles: {role_name: "Get Schedule Email"}).map(&:user_email).join(",")
  UserMailer.admin_schedule_email(@users,@schedule).deliver 
end
...
task :send_update_schedule_mail => :environment do
  ...
  @users=User.joins(:roles).where(roles: {role_name: "Get Schedule Email"}).map(&:user_email).join(",")
  UserMailer.admin_update_schedule_email(@users,@schedule).deliver  
end
...
task :send_result_mail => :environment do
  ...
  @users=User.joins(:roles).where( roles: { role_name: "Get Selection Email"}).map(&:user_email).join(",")
  UserMailer.admin_result_email(@users,@results).deliver 
end
desc "Send selected result email"
task :send_selected_result_mail => :environment do
  ids = ENV["CANDIDATE_IDS"].split(",")
  @results = RecruitmentTest.where(id: ids ).order("mark_percentage DESC")
  @users=User.joins(:roles).where( roles: { role_name: "Get Selection Email"}).map(&:user_email).join(",")
  UserMailer.admin_selected_result_email(@users,@results).deliver 
end
